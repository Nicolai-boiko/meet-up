...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res
      .status(503)
      .json({ status: 'DOWN', message: 'Server is running but database connection failed.' })
  }
})
...
// Запускаем сервер только если файл запущен напрямую (не импортирован для тестов)
