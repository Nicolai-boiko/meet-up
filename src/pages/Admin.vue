            ...
            <table class="w-full text-sm">
              <thead class="bg-gray-50 dark:bg-gray-800 border-b border-gray-200">
                <tr>
                  ...
                  >
                    Email
                    <span v-if="userSortBy === 'email'" class="ml-1 text-blue-500">{{
                      userSortOrder === 'asc' ? '↑' : '↓'
                    }}</span>
                  </th>
                  <th
                ...
                <tr v-for="user in adminUsers.items" :key="user.id">
                  <td class="px-4 py-3">
                    <div class="flex items-center gap-2">
                      ...
                      <div
                        v-else
                        class="w-7 h-7 rounded-full bg-blue-400 flex items-center justify-center text-[11px] font-bold text-white"
                      >
                        {{ userInitials(user) }}
                      </div>
          ...
          >
            <template v-if="selectedContentIds.length">
              <span class="text-sm text-red-700">
                Выбрано: {{ selectedContentIds.length }}
                {{ pluralize(selectedContentIds.length, 'материал', 'материала', 'материалов') }}
              </span>
              <button
              ...
              <tr>
                <th class="w-10 px-2 py-3">
                  <input
                    type="checkbox"
                    :checked="
                      selectedContentIds.length === adminContent.items.length &&
                      adminContent.items.length > 0
                    "
          ...
          >
            <template v-if="selectedRoomIds.length">
              <span class="text-sm text-red-700">
                ...
                {{ pluralize(selectedRoomIds.length, 'комната', 'комнаты', 'комнат') }}
              </span>
              <button
              ...
              <tr>
                <th class="w-10 px-2 py-3">
                  <input
                    type="checkbox"
                    :checked="
                      selectedRoomIds.length === adminRooms.items.length &&
                      adminRooms.items.length > 0
                    "
