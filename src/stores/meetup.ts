...
export const useMeetupStore = defineStore('meetups', () => {
  ...
  async function fetchMeetups(userIds?: number[]) {
    loading.value = true
    try {
      const params: MeetupQueryParams = { page: 1, limit }
  ...
  async function fetchAllForCalendar(userIds?: number[]) {
    loading.value = true
    try {
      const params: MeetupQueryParams = { page: 1, limit: 200 }
      const ids = userIds ?? selectedUserIds.value
      if (ids.length > 0) {
        params.userIds = ids.join(',')
      }
  ...
  async function fetchMore() {
    ...
    try {
      ...
      const params: MeetupQueryParams = { page: nextPage, limit }
  ...
  async function updateMeetup(
    ...
    const { data } = await apiClient.put<{ message: string; data: Meetup }>(
      `/meetups/${id}`,
      payload,
    )
  ...
  async function declineMeetup(id: number) {
    const { data } = await apiClient.post<{ message: string; data: Meetup }>(
      `/meetups/${id}/decline`,
    )
    const idx = items.value.findIndex((m) => m.id === id)
    if (idx >= 0) items.value[idx] = data.data
